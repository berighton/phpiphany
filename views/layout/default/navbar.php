...
function render_navbar(&$view) {
	...
	if ($view->navbar_login){
		if (isset($_SESSION['user_guid']) and $_SESSION['user_guid']){
			...
			$text = html::login(array('assets_dir' => $view->assets_dir, 'token' => $token));
		}
...
HTML;
		...
		foreach ($view->navbar_menu as $resource) {
			if (!isset($resource['type']) or $resource['type'] != 'dropdown'){
				$links .= '					<li';
				$links .= (isset($resource['active']) and $resource['active'])? ' class="active">' : '>';
...
<div class="navbar navbar-fixed-top">
	<div class="navbar-inner">
		<div class="container">
			...
			<!-- <a class="brand" style="padding: 7px 10px 0 20px" href="$config->site_url">$config->site_name</a> -->
