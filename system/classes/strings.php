<?php
/**
 * Strings class taken from Fuel framework
 *
 * (currently not used)
 *
 * ******************************************************
...
class strings {
	...
	public static function ascii($str) {
		/*
		$output = '';
		// Translate each character in a string to its ASCII counterpart
		foreach ($str as $char){
			$output .= mb_convert_encoding('&#' . intval($u) . ';', 'UTF-8', 'HTML-ENTITIES');
			//or
			$output .= chr(ord($char));
		}
		*/
