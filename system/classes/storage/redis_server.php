<?php
/**
 * redis_server allows you to work with Redis storage in PHP
 *
 ...
 */
class redis_server implements iredis_server {
	...
	public function connect($host, $port) {
		...
		if (!$socket) throw new error("Unable to connect to Redis server. <strong>Error $errno: $errstr ('$host' on port $port)</strong>");
	...
	protected function _send($args) {
		...
		if (!$w) {
			...
			if (!fwrite($this->connection, $command)) {
				global $view;
				$view->system_msg = $view->error('<strong>$redis->_send()</strong> command was not sent', 'error');
				return false;
			}
		}
	...
	protected function _read_reply() {
		...
		switch ($reply[0]) {
			/* Error reply */
			case '-':
				$view->system_msg = $view->error('$redis->_read_reply() error: ' . $reply);
				return false;
			/* Inline reply */
			case '+':
			...
			case '$':
				...
				if ($size > 0) {
					do {
						...
						if ($data === false) {
							$view->system_msg = $view->error('$redis->_read_reply() error when reading answer');
							return false;
						}
						$response .= $data;
						$read += $block_size;
			...
			case ':':
				...
				break;
			default:
				$view->system_msg = $view->error('$redis->_read_reply() non-protocol answer: ' . print_r($server_reply, 1));
				return false;
		}
