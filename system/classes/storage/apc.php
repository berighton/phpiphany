...
class pip_apc extends memory implements imemory {
	...
	public function add($k, $v, $ttl = 259200, $tags = NULL) {
		global $view;
		if (empty($k)) {
			$view->system_msg = $view->error('APC: empty keys are not allowed in <em>$apc->add("' . $k . '")</em> method');
			return false;
		}
		...
		if (!$add) {
			if (!apc_exists($this->prefix . $k)) {
				$view->system_msg = $view->error('APC: can not add non existing key in <em>$apc->add("' . $k . '")</em> method');
			}
			return false;
		}
		if (!empty($tags)) {
			$this->set_tags($k, $tags, $ttl);
		}
		return true;
	}
	 ...
	 * @return bool
	 */
	public function set_tags($key, $tags, $ttl = -1) {
		if (!is_array($tags)) {
			if (is_scalar($tags)) {
				$tags = array($tags);
			} else {
	...
	public function save($k, $v, $ttl = 259200, $tags = NULL, $serialize = true) {
		global $view;
		if (empty($k)) {
			$view->system_msg = $view->error('APC: empty keys are not allowed in <em>$apc->save("' . $k . '")</em> method');
			return false;
		}
		...
		if (!apc_store($this->prefix . $k, $v, intval($ttl))) {
			$view->system_msg = $view->error('APC: can not store key in <em>$apc->save("' . $k . '")</em> method');
			return false;
		}
	...
	public function read($k, &$ttl_left = -1, $serialize = true) {
		global $view;
		if (empty($k)) {
			$view->system_msg = $view->error('APC: empty keys are not allowed in <em>$apc->read("' . $k . '")</em> method');
			return NULL;
		}
		if (is_array($k)) {
			$data = array();
			...
			if (!$success) {
				if (apc_exists($this->prefix . $k)) {
					$view->system_msg = $view->error('APC: can not fetch key in <em>$apc->read("' . $k . '")</em> method');
				}
				return false;
			}
			if ($ttl_left !== -1) {
	...
	public function del($k) {
		if (empty($k)) {
			global $view;
			$view->system_msg = $view->error('APC: empty keys are not allowed in <em>$apc->del("' . $k . '")</em> method');
			return false;
		}
		...
		if (is_array($k)) {
			...
			if (apc_exists($this->tags_prefix . $k)) {
				apc_delete($this->tags_prefix . $k);
			}
			if (apc_exists($this->lock_key_prefix . $k)) {
				apc_delete($this->lock_key_prefix . $k);
			}
	...
	public function increment($key, $by_value = 1, $limit_keys_count = 0, $ttl = 259200) {
		if (empty($key)) {
			global $view;
			$view->system_msg = $view->error('APC: empty keys are not allowed in <em>$apc->increment("' . $key . '")</em> method');
			return false;
		}
	...
	public function unlock_key(locker $auto_unlocker) {
		...
		if (empty($key)) {
			global $view;
			$view->system_msg = $view->error('APC: empty key in the AutoUnlocker in <em>$redis->unlock_key("' . $key . '")</em> method');
			return false;
		}
	...
	public function get_stat() {
		return array('system' => apc_cache_info('', true), 'user' => apc_cache_info('user', true));
	}
